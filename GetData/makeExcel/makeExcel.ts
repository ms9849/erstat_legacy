...
async function DataToExcel(tblName) {
    let connection, rows, cols, chrData, allCnt;

    let character = JSON.parse(fs.readFileSync("./json/character.json").toString());
    let weapon = JSON.parse(fs.readFileSync("./json/weapon.json").toString());
    ...
    [ rows, cols ] =  await connection.query(`SELECT 
                characterNum, 
                bestWeapon, 
                sum(top1) as top1, 
                sum(top3) as top3, 
    ...
    await jexcel.j2e(data,function(err){ 
